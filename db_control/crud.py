...
def myinsert(mymodel, values):
    # idを必須にするために追加
    if "customer_id" not in values or not values["customer_id"]: # 空文字列もFalseと評価される
        # (実際には、ここで例外を発生させるか、エラーを示す値を返す方が良い)
        print(f"エラー: {mymodel.__name__} の customer_id は必須であり、空にできません。")
        return None
...
def myupdate(mymodel, values):
    ...
    query = (
        update(mymodel)
        .where(mymodel.customer_id == customer_id) # どの顧客を更新するか指定
        .values(**values)  # どの値を更新するか指定 (values辞書を展開)
    )
    try:
        # トランザクションを開始
        with session.begin():
